...
public:
  ...
  /// \return The dereferenced term.
  const Term& operator[](difference_type n) const { return *(m_term + n); }
  ...
  ptrdiff_t operator-(const term_appl_iterator& other) const { return m_term - other.m_term; }

  /// \brief Provide the distance to the other iterator.
  /// \param other the other iterator.
  /// \return the distance from other to this iterator.
  ptrdiff_t distance_to(const term_appl_iterator& other) const { return other.m_term - m_term; }
  ...
  bool operator==(const term_appl_iterator& other) const { return m_term == other.m_term; }

  /// \brief Comparison of iterators.
  /// \param other The iterator with which this iterator is compared.
  /// \return true if the pointer to this termterm is smaller than the other pointer.
  std::weak_ordering operator<=>(const term_appl_iterator& other) const { return m_term <=> other.m_term; }
