...
#else
  export module atermpp:detail.thread_aterm_pool;
...
public:
  ...
  void create_appl_dynamic(aterm& term,
  ...
  mcrl2::utilities::shared_guard lock_shared() { return m_shared_mutex.lock_shared(); }

  /// Acquire an exclusive lock
  mcrl2::utilities::lock_guard lock() { return m_shared_mutex.lock(); }

  /// Returns true iff we are in a shared section.
  bool is_shared_locked() { return m_shared_mutex.is_shared_locked(); }

  /// Triggers a global garbage collection
  void collect() { m_pool.collect(m_shared_mutex); }
...
#include "thread_aterm_pool_implementation.cxx"
